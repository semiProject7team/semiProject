<%@page import="com.javaclass.basic.vo.SizeVO"%>
...
<body>
					...
					<div class="row">
						<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
							<div class="page-header">
								<h2 class="pageheader-title">용기사이즈관리</h2>
							</div>
							...
							<div class="card-body border-top col-sm-2">
								<select class="form-control" id="selectoption">
                                    ...
                                    <option>용기사이즈 내림차순</option>
                                    <option>용기사이즈 오름차순</option>
                                </select>
							</div>
						</div>
					</div>
											...
											<table class="table">
												<thead class="bg-light">
													<tr class="border-0">
														...
														<th class="border-0">사이즈번호</th>
													</tr>
												</thead>
												<tbody id="tbody">
													<c:forEach var="vo" items="${getPageSizeList}">
														<tr>
															<td>${vo.no}</td>
															<td>${vo.goodsSize}</td>
															<td>${vo.goodsSizeNo}</td>
														</tr>
													</c:forEach>
												</tbody>
											</table>
                            ...
                            <div class="row">
								<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                                    <nav aria-label="Page navigation example">
                                    	<!-- 테이블 충 레코드수 -->
										<c:set var="recordsCnt" value="${getSizeCount.no}" />
										<c:set var="jspFile" value="manager_goodsSize.do?" />
										<!-- include 페이징  jsp파일  -->
										<%@include file="manager_paging.jsp" %>
									  	<a href="manager_goodsSize_insert.do" class="btn btn-outline-light float-left">용기사이즈추가</a>
									  	<a href="manager_goodsSize_delete.do" class="btn btn-outline-light float-left">용기사이즈삭제</a>
                                    </nav>                             
								</div>
							</div>
						</div>
	...
	<script type="text/javascript"
		src="/7team_project/resources/js/manager/manager_goodsSize.js"></script>
